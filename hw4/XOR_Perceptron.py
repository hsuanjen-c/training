...
def gradient_W1(W1):
    ...
    for Xi,yi in zip(X,y):
        ...
        error = A2-yi
        dLdZ2 = error * A2 * (1 - A2)
        #dLdZ2 = sigmoid(Z2)*(1-sigmoid(Z2))*0.5*error #1,1
...
def gradient_b1(b1):
    ...
    for Xi,yi in zip(X,y):
        ...
        dLdZ2 = error * A2 * (1 - A2)
        #dLdZ2 = sigmoid(Z2)*(1-sigmoid(Z2))*0.5*error #1,1
        dLdZ1 = W2.T@dLdZ2*sigmoid(Z1)*(1-sigmoid(Z1))
        gradeint_i+=dLdZ1
    return gradeint_i/4
...
while counter:
    ...
    b1 = b1-learning_rate*gradient_b1(b1)
    loss = loss_function()
    if not (counter%10000):print(counter,":",loss)
for Xi,yi in zip(X,y):
    Xi = Xi.reshape(2,1)
    print("X:",Xi)
